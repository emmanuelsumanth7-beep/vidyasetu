...
function letterGrade(pct: number) {
  ...
  if (pct >= 80) return { letter: 'A',  color: '#1B2A4A', bg: 'rgba(27,42,74,0.10)' };
    ...
    setTimeout(() => {
      ...
      if (user?.schoolId) {
        sendNotification({
          ...
          body: `Marks have been published by ${user.name || 'a teacher'}.`,
    ...
    <div className="max-w-[1200px] mx-auto w-full animate-fade-in pb-20 space-y-6 relative">
      ...
      {marks.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total Records', value: String(marks.length), color: '#1B2A4A', icon: FileText },
            { label: 'Avg Class Score', value: '82%', color: '#34C759', icon: TrendingUp },
            { label: 'Highest Score', value: '96%', color: '#C49B2A', icon: Award },
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-5 md:px-6 flex items-center justify-between" style={{ borderBottom: '1px solid var(--color-border)' }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'rgba(88,86,214,0.1)', color: '#C49B2A' }}>
              <Award size={16} />
            </div>
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2 text-sm font-bold" style={{ color: 'var(--color-text-primary)' }}>
                        <FileText size={14} style={{ color: '#C49B2A' }} />
                        {m.examName}
                      </div>
                    </td>
