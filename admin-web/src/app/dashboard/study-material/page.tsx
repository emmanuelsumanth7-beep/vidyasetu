...
import { Upload, BookOpen, CheckCircle2, X, Link as LinkIcon, Video } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl mx-auto pb-32">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-ink-primary font-display tracking-tight">Upload Study Material</h1>
          <p className="text-sm font-medium text-ink-secondary mt-1">Share PDFs, notes, and video resources with your students.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8 bg-white p-8 rounded-[32px] shadow-sm border border-gray-100">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-1/4 flex flex-col gap-2">
            <label className="text-xs font-bold uppercase tracking-widest text-ink-secondary">Material Type</label>
            <select 
              value={type}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div 
            ...
            onDrop={handleDrop}
            className={`relative flex flex-col items-center justify-center p-12 border-2 border-dashed rounded-[32px] transition-all ${isDragging ? 'border-emerald-500 bg-emerald-50' : 'border-gray-300 bg-gray-50 hover:bg-gray-100'}`}
          >
            {file ? (
              <div className="flex flex-col items-center gap-4">
                <div className="w-16 h-16 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center justify-center text-emerald-500">
                  {type === 'video' ? <Video size={32} /> : type === 'link' ? <LinkIcon size={32} /> : <BookOpen size={32} />}
                </div>
              ...
              <>
                <div className="w-20 h-20 bg-white rounded-full shadow-sm border border-gray-100 flex items-center justify-center text-emerald-500 mb-6">
                  <Upload size={32} />
                </div>
                <p className="text-sm font-bold text-ink-primary">Drag & drop your resource here</p>
        ...
        <div className="pt-4 border-t border-gray-100 flex justify-end">
          <button 
            ...
            className="px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg"
          >
            {status === 'uploading' ? (
            ...
            {status === 'uploading' ? 'Publishing...' : status === 'success' ? 'Published!' : 'Upload Material'}
          </button>
        </div>
