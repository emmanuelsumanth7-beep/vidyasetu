...
router.get('/documents', authenticate, async (req, res) => {
  try {
    const docs = await prisma.documentApproval.findMany({
      where: { schoolId: req.user.schoolId },
...
router.post('/messages', authenticate, async (req, res) => {
    try {
        ...
        const msg = await prisma.message.create({
            data: { schoolId: req.user.schoolId, senderId: req.user.id, receiverId, content }
        });
        res.json(msg);
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/notices', authenticate, async (req, res) => {
    try {
        ...
        const validAudience = ['ALL_STAFF', 'ALL_PARENTS', 'ALL_STUDENTS', 'SPECIFIC_CLASS', 'SPECIFIC_STAFF', 'EVERYONE'].includes(audience) ? audience : 'EVERYONE';
        const notice = await prisma.notice.create({
            data: { schoolId: req.user.schoolId, title, body, audience: validAudience, postedById: req.user.id }
        });
        res.json(notice);
    } catch (e) { res.status(500).json({ error: e.message }); }
});
